...
export class AddPatientMComponent {
  ...
  public document_type_id: number | null = 1; // Agregado para el tipo de documento
  ...
  ngOnInit(): void {
    ...
    // })
    this.patientService.getDocumentTypes().subscribe((response: any) => {
      if (response.total>0) {
          this.documentTypes = response.document_types.data;  // Suponiendo que la respuesta tiene una propiedad 'data' con los tipos de documento
      } else {
          console.error("Error al cargar los tipos de documentos", response.message);
      }
  }, (error) => {
  ...
  save() {
    ...
    if (!this.name || !this.n_document || !this.surname || !this.mobile) {
      this.text_validation = "LOS CAMPOS SON NECESARIOS (Nombre, Apellido, N° documento, Teléfono)";
      return;
    }
    if (!this.name_companion || !this.surname_companion) {
      this.text_validation = "EL NOMBRE Y APELLIDO DEL ACOMPAÑANTE SON OBLIGATORIOS";
      return;
    }
    ...
    // Agregar el tipo de documento seleccionado al FormData
    formData.append("document_type_id", this.document_type_id?.toString() || '');
